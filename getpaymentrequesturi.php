...
function createPaymentRequest($params, &$formErrors)
{
	...
	if ($cachedCertChain === FALSE) {
            $leafCert = file_get_contents("/home/murcielago/.certs/".$params['merchant'].".crt");
        ...
        $priv_key = file_get_contents("/home/murcielago/.certs/".$params['merchant'].".key");
	...
	if ($nAddresses == 1 && $totalAmount > 0) {
        ...
        if ($nAddresses == 1) {
            $url = AddArgsToURL("dash:".$params["address1"], $urlParams);
        }
        else {
            $url = AddArgsToURL("dash:", $urlParams);
	}
        return $url;
    }

    header('Content-Type: application/dash-paymentrequest');
    $filename = "r".(string)time().".dashpaymentrequest";
    header('Content-Disposition: inline; filename='.$filename);
    header('Content-Transfer-Encoding: binary');
    header('Expires: 0');
    header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
    header('Pragma: public');
    header('Content-Length: ' . (string)strlen($url));
    echo $url;
